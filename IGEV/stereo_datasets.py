...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root='data/sceneflow/', dstype='frames_finalpass', things_test=False):
        super(SceneFlowDatasets, self).__init__(aug_params)  #只用dribing
        self.root = root
        self.dstype = dstype
    ...
    def _add_driving(self, split="TRAIN"):
        ...
        root = self.root 
        # left_images = sorted( glob(osp.join(root, self.dstype, split, '*/*/*/left/*.png')) )
        data_dir = os.path.join(root,'driving')
        #import pdb; pdb.set_trace()
        left_images = sorted(glob(data_dir + '/' + self.dstype + '/*/*/*/left/*.png'))
        right_images = [ image_file.replace('left', 'right') for image_file in left_images ]
        disparity_images = [ im.replace(self.dstype, 'disparity').replace('.png', '.pfm') for im in left_images ]
...
class KITTI(StereoDataset):
    def __init__(self, aug_params=None, root='data/kitti_15', image_set='training'):
        super(KITTI, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispKITTI)
        assert os.path.exists(root)

        root_12 = 'data/kitti_15'
...
def fetch_dataloader(args):
    ...
    train_loader = data.DataLoader(train_dataset, batch_size=args.batch_size, 
        pin_memory=True, shuffle=True, num_workers=16, drop_last=True)  #int(os.environ.get('SLURM_CPUS_PER_TASK', 6))-2
