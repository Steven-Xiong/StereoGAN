...
def train(args):
    ...
    if args.pcwnet:
        net = __models__[args.model](args.maxdisp)
    else:                 
    ...
    if args.pcwnet:
        optimizer = optim.Adam(net.parameters(), lr=args.lr_rate,  #调一样不要整混
                                betas=(0.9, 0.999))
    else:
    ...
    for epoch in range(start_epoch,args.total_epochs):
        ...
        for i, batch in enumerate(trainloader):
            ...
            if args.pcwnet:
                ...
                disp = dispA.squeeze(1)
                mask = maskA.squeeze(1)
                disp_loss = model_loss(disp_ests,disp,mask)
                ...
                if args.lambda_disp_warp_inv:
                    ...
                    #print(fake_leftA_feats[0][0].shape)
                    loss_disp_warp_inv = G_BA_debug(rightB, disp_warp, True, [x.detach() for x in fake_leftA_feats])
                    #print(loss_disp_warp_inv)
                    #import pdb; pdb.set_trace()
...
if __name__ == '__main__':
    ...
    # use pcwnet stereo part
    parser.add_argument('--pcwnet',type = float,help= 'use pcwnet as dispnet', default = 1)
